...
reference/roster.csv             — staff_id column, one per row — auth
                                    data, NOT marking content, NOT sent to
                                    the model under any circumstances
...
- **Reference bundle**: only `.md` files from `reference/` are concatenated into what's sent to Claude. `reference/roster.csv` is explicitly excluded — filter by extension, don't just glob the whole directory.
- **Login gate**: staff ID matched against `reference/roster.csv` — no access code, no password, no OIDC or external identity provider. Happens entirely before the chat renders, never inside the conversation; the model has no role in authentication.
