...
- Existing null delivery dates (order_delivered_carrier_date, order_delivered_customer_date) are null for cancelled orders in all datasets. This is a data characteristic, not a quality issue. Credit students who correctly identify "no action required" with justification. Do not credit imputation or removal of these rows as one of the two required activities. **If a group imputes these fields anyway, that is not a neutral, doesn't-count-either-way action — the null is the true, correct state (the order was never delivered), and replacing it with a fabricated placeholder date introduces false data into the dataset, not a harmless extra step.** Confirming the code matches the report's description of what was done is still correct and worth stating — but "implemented as described" is not the same as "this was a sound decision," and this specific action is not a sound one. Flag it as a genuine cleansing-judgment concern weighed into 2.2's own band, even though it fills neither of the two required activity slots, and separately flag it as a fact worth checking wherever these date fields are relied on later — a DAX formula computing delivery or processing time from them would silently produce a nonsensical result for every cancelled order (a multi-decade gap instead of correctly excluding them).
