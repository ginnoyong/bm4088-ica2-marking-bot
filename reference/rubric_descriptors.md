...
- First Page present — introduces the business context and objectives, framing why the analysis matters to the decision-maker. Check content, not just presence: does the framing on this page actually match the problem statement in the Written Report (1.1), or does it describe a different business problem, decision-maker, or objective? Get the actual text on this page, verbatim (Power BI text elements are selectable, so a marker can copy-paste directly rather than retyping or screenshotting) — a divergence here is a technical mismatch between the two artifacts, per Marking Principle 4, not a separate penalty.
...
- Conclusion and Recommendations page present, with exactly TWO (2) actionable recommendations. Check content, not just presence: get the actual text on this page, verbatim (same reasoning as the First Page above — copy-paste directly from Power BI, don't screenshot or retype), and compare it against the Written Report's own Conclusion and Recommendations section (5.2). A divergence between the two versions of the conclusion is a technical mismatch, per Marking Principle 4.
